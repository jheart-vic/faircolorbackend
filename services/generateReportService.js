...
const statusColor = (s) =>
    s === 'approved' ? C.success : s === 'rejected' ? C.danger : C.warning
const typeColor = (t) =>
    t === 'deposit' ? C.success : t === 'withdrawal' ? C.danger : C.accent
...
function drawBanner(doc, leftLines, rightLines) {
    ...
    let ly = MARGIN + 14
    leftLines.forEach(
        ({ text: t, size = 8, color = '#A8C4E8', font = 'Helvetica' }) => {
...
function drawFooter(doc) {
    ...
    txt(
        doc,
        'FairColor Bank  -  This is a system-generated report and does not require a signature.',
        0,
        ph - 26,
        { size: 7, color: C.white, width: pw, align: 'center' },
    )
    txt(doc, 'Confidential - For authorized use only', 0, ph - 14, {
        size: 7,
        color: '#A8C4E8',
        width: pw,
        align: 'center',
...
function drawTransactionRows(doc, transactions, cols, y, withCustomer = false) {
    ...
    transactions.forEach((t, i) => {
        ...
        if (withCustomer) {
            txt(doc, fdate(t.createdAt), cx + 5, vc, {
                size: 7.5,
                width: cols[1].w - 8,
            })
            cx += cols[1].w
            txt(doc, t.customerId?.fullName || '—', cx + 5, vc, {
                size: 7.5,
                width: cols[2].w - 8,
            })
            cx += cols[2].w
            txt(doc, ttype.toUpperCase(), cx + 5, vc, {
                size: 8,
                color: typeColor(ttype),
                font: 'Helvetica-Bold',
                width: cols[3].w - 8,
                align: 'center',
            })
            cx += cols[3].w
            txt(doc, fc(t.amount), cx + 5, vc, {
                size: 8,
                color: typeColor(ttype),
                font: 'Helvetica-Bold',
                width: cols[4].w - 8,
            ...
            cx += cols[4].w
            txt(doc, t.note || '—', cx + 5, vc, {
                size: 7.5,
                width: cols[5].w - 10,
            })
            cx += cols[5].w
            txt(doc, (t.status || '').toUpperCase(), cx + 5, vc, {
                size: 7,
            ...
            txt(doc, fdatetime(t.createdAt), cx + 5, vc, {
                size: 7.5,
                width: cols[1].w - 8,
            })
            cx += cols[1].w
...
function drawTransactions(doc, transactions, y, withCustomer = false) {
    ...
    if (!transactions.length) {
        ...
        txt(doc, 'No transactions found for this period.', MARGIN, y + 12, {
            size: 9,
            color: C.grey,
            width: bW(doc),
            align: 'center',
...
function drawLoans(doc, loans, y) {
    ...
    if (!loans.length) {
        drawBorderRect(doc, MARGIN, y, bW(doc), 36, C.lightGrey, C.border)
        txt(doc, 'No loans found for this period.', MARGIN, y + 12, {
            size: 9,
            color: C.grey,
            width: bW(doc),
            align: 'center',
    ...
    loans.forEach((loan, i) => {
        ...
        txt(doc, String(i + 1), cx + 5, vc, {
            size: 8,
            width: LOAN_COLS[0].w - 8,
            align: 'center',
        })
        cx += LOAN_COLS[0].w
        txt(doc, loan.publicId || '—', cx + 5, vc, {
            size: 8,
            width: LOAN_COLS[1].w - 8,
        })
        cx += LOAN_COLS[1].w
        txt(doc, fc(loan.amount), cx + 5, vc, {
            size: 8,
            color: C.accent,
            font: 'Helvetica-Bold',
            width: LOAN_COLS[2].w - 8,
            align: 'right',
        })
        cx += LOAN_COLS[2].w
        txt(doc, `${loan.interest || 0}%`, cx + 5, vc, {
            size: 8,
            width: LOAN_COLS[3].w - 8,
            align: 'center',
        })
        cx += LOAN_COLS[3].w
        txt(doc, `${loan.duration || 0} months`, cx + 5, vc, {
            size: 8,
            width: LOAN_COLS[4].w - 8,
            align: 'center',
        })
        cx += LOAN_COLS[4].w
        txt(doc, (loan.status || '').toUpperCase(), cx + 5, vc, {
            size: 7,
            color: statusColor(loan.status),
            font: 'Helvetica-Bold',
            width: LOAN_COLS[5].w - 8,
            align: 'center',
        })
        cx += LOAN_COLS[5].w
        txt(doc, fdate(loan.createdAt), cx + 5, vc, {
            size: 8,
            width: LOAN_COLS[6].w - 8,
            align: 'center',
        })
        y += rowH
    })
...
export async function generateCustomerReport(customerId, user, res, query) {
    ...
    let y = drawBanner(
        ...
        [
            {
                text: 'CUSTOMER FINANCIAL REPORT',
                size: 11,
                color: C.white,
                font: 'Helvetica-Bold',
            },
...
export async function generateCashierReport(query, adminUser, res) {
    ...
    const balance = {
        ...
        withdrawals,
        loans,
        balance: deposits - withdrawals - loans,
    }
    ...
    let y = drawBanner(
        ...
        [
            {
                text: 'CASHIER FINANCIAL STATEMENT',
                size: 11,
                color: C.white,
                font: 'Helvetica-Bold',
            },
    ...
    return res.json({
        ...
        data: {
            ...
            transactions: transactions.map((t) => ({
                id: t._id,
                date: t.createdAt,
                customer: t.customerId?.fullName || '—',
                customerId: t.customerId?.publicId || '—',
                type: t.type,
                amount: t.amount,
                note: t.note || '—',
                status: t.status,
...
export async function getCustomerReportData(customerId, user, res, query) {
  const { filter, startDate, endDate } = query;
  ...
  return res.json({
    ...
    data: {
      ...
      guarantor: customer.guarantor?.fullName ? {
        fullName: customer.guarantor.fullName,
        phone: customer.guarantor.phone || "—",
        relationship: customer.guarantor.relationship || "—",
